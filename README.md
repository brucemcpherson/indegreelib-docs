...
4.  **Data Ingestion**: The application collects the form responses and processes them into a graph data structure, consisting of 'vertices' (the participants) and 'edges' (the relationships and their ratings between participants). It also automatically tracks respondent participation across phases (e.g., `responded_pre`, `responded_phase_count`). This is handled by `Googly.js` and `ResponseHandler.js`.
...
*   **Treatment & Network Contagion Analysis**: `Network.js` provides graph analysis using Cytoscape.js, including treatment classification mapping, centrality metrics (`betweenness`, `closeness`, `eigenvector`), bottleneck chokepoint ranking (`bottleneck_rank`), diversified greedy composite seed selection (`base_seed_score`, `seed_rank`), pre-stage network capacity assessment (influencer reach, unreachable percentage, viability checks), and distance tracking to treated nodes (`treated_distance`, `treated_id`). Detailed metric descriptions and formulas are available in [vertex_attributes.md](file:///Users/brucemcpherson/Documents/repos/indegreelib/docs/vertex_attributes.md).
